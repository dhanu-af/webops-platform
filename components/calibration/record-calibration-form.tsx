...
export function RecordCalibrationForm({
  ...
  if (!open) {
    return (
      <Button size="sm" onClick={() => setOpen(true)}>
        <PlusCircle className="size-4" /> Record Calibration
      </Button>
    );
  }
  ...
  return (
    <Card>
      <CardHeader>
        <div>
          <CardTitle>Record a Calibration</CardTitle>
          <CardDescription>
            Log the date, interval, and vendor for this equipment
          </CardDescription>
        </div>
      </CardHeader>
      <CardContent>
        <form ref={formRef} action={handleSubmit} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              <label className="text-xs font-medium text-muted-strong">
                Certificate (PDF, JPG, or PNG — optional)
              </label>
              <input
                type="file"
                name="certificate"
