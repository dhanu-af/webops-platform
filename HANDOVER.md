...
- **5S Daily Check rollout**: "Capsule Room", "Blending Room", and now "Bottling Line" clones all have correctly area-scoped `DAILY · SUPERVISOR` schedules and appear on `/five-s` as `DAILY · Capsule Room` / `DAILY · Blending Room` / `DAILY · Bottling Area` respectively. (Blending Room's was originally facility-wide; user fixed it herself via Pause + Add Schedule after automation was blocked from touching an existing production schedule.)
- **Bottling Line's Pre-Start and Post-Operation Cleaning checklists still don't exist** — unlike 5S (generic content, safe to clone), those are equipment/process-specific and need the user's real paper form content before they can be built. Don't fabricate compliance checklist items.
...
1. **Get Bottling Line's real Pre-Start and Post-Operation Cleaning paper forms from the user** — needed before those two checklists can be built (5S is already done, see State). Ask her directly; don't guess at equipment-specific content.
