...
export default async function EquipmentCalibrationDetailPage({
  ...
  const equipment = await db.equipment.findUnique({
    ...
    include: { area: { select: { name: true, section: { select: { name: true } } } } },
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/calibration" className="inline-flex items-center gap-1.5 text-xs font-medium text-muted hover:text-foreground">
          <ArrowLeft className="size-3.5" /> All Equipment
        </Link>
        <div className="mt-2 flex items-center gap-3">
          <h1 className="text-xl font-semibold tracking-tight text-foreground">{equipment.name}</h1>
          <Badge tone={statusMeta.tone}>{statusMeta.label}</Badge>
        </div>
        <p className="text-sm text-muted">
          {equipment.area.section.name} / {equipment.area.name} · {equipment.code}
      ...
      <Card>
        ...
        <CardContent className="pt-2">
          {history.length === 0 ? (
            <p className="py-8 text-center text-sm text-muted">No calibration recorded yet for this equipment.</p>
          ) : (
            <div className="divide-y divide-border">
              {history.map((h) => (
                <div key={h.id} className="flex items-center justify-between gap-4 py-3.5">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-foreground">
                      Calibrated {format(h.calibratedDate, "d MMM yyyy")} by {h.performedBy}
                    </p>
                    <p className="mt-1 text-xs text-muted">
                      Due {format(h.dueDate, "d MMM yyyy")} ({h.intervalDays}-day interval)
                      {h.certificateNumber && <> · Cert #{h.certificateNumber}</>}
